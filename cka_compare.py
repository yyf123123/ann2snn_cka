...
class CKA:
    ...
    也提供了可用于训练的静态方法（similarity 和 cka_train）。
    """
    ...
    @staticmethod
    def similarity(X: torch.Tensor, Y: torch.Tensor, kernel: str = 'linear', epsilon: float = 1e-9) -> torch.Tensor:
        """
        计算两个特征矩阵X和Y之间的CKA相似度 (标准版本)。
        """
        X = X.view(X.size(0), -1)
        Y = Y.view(Y.size(0), -1)
        ...
        cka_val = hsic_numerator / (torch.sqrt(hsic_denom_X * hsic_denom_Y) + epsilon)
        return cka_val

    # =============================================================================
    #           NEW MEMORY-EFFICIENT METHOD FOR TRAINING
    # =============================================================================
    @staticmethod
    def cka_train(X: torch.Tensor, Y: torch.Tensor, kernel: str = 'linear', epsilon: float = 1e-9) -> torch.Tensor:
        """
        一个内存高效的CKA计算版本，专为训练设计。
        它通过在 `no_grad` 上下文中计算格拉姆矩阵来防止内存泄漏，同时保留梯度流。
        """
        hsic_numerator = 0.
        hsic_denom_X = 0.
        hsic_denom_Y = 0.

        # --- 在 no_grad 上下文中计算，以防止中间张量（格拉姆矩阵）持有计算图 ---
        with torch.no_grad():
            X_flat = X.view(X.size(0), -1)
            Y_flat = Y.view(Y.size(0), -1)

            if kernel == 'linear':
                gram_X = torch.matmul(X_flat, X_flat.t())
                gram_Y = torch.matmul(Y_flat, Y_flat.t())
            else:
                raise NotImplementedError("RBF kernel not implemented yet.")

            centered_gram_X_no_grad = CKA._center_gram(gram_X)
            centered_gram_Y_no_grad = CKA._center_gram(gram_Y)
        
        # --- 重新引入梯度 ---
        # 这一步创建了一个非常小的计算图，直接从HSIC值连接回原始的X和Y
        hsic_numerator = (CKA._center_gram(torch.matmul(X.view(X.size(0), -1), X.view(X.size(0), -1).t())) * centered_gram_Y_no_grad).sum() + \
                         (centered_gram_X_no_grad * CKA._center_gram(torch.matmul(Y.view(Y.size(0), -1), Y.view(Y.size(0), -1).t()))).sum()
        
        hsic_denom_X = (centered_gram_X_no_grad * centered_gram_X_no_grad).sum()
        hsic_denom_Y = (centered_gram_Y_no_grad * centered_gram_Y_no_grad).sum()
